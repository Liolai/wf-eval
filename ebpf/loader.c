...
static void cleanup(int sig) {
    __u32 key = 0;
    struct state state;
    if (map_fd_global >= 0 && bpf_map_lookup_elem(map_fd_global, &key, &state) == 0) {
        printf("\nFinal State\n");
...
int main(int argc, char** argv) {
            ...
            case 0:
                if (strcmp("mode", long_options[option_index].name) == 0) {
                    if (strcmp("dynamic", optarg) == 0)
                        ...
                        mode = MODE_FIXED;
                    else if (strcmp("ingress", optarg) == 0)
                        mode = MODE_INGRESS;
                    else if (strcmp("dummy", optarg) == 0 || strcmp("dummy_fixed", optarg) == 0)
                        mode = MODE_DUMMY;
                    else if (strcmp("dummy_dynamic", optarg) == 0)
                        mode = MODE_DUMMY_DYNAMIC;
                    else if (strcmp("combined", optarg) == 0)
                        mode = MODE_COMBINED;
                    else {
    ...
    if (!eg_prog) {
        fprintf(stderr, "Error: handle_egress not found\n");
        return 1;
    }
    ...
    if (ing_prog && mode != MODE_DUMMY && mode != MODE_DUMMY_DYNAMIC) {
        ...
        if (bpf_tc_attach(&hook, &ing_opts)) {
    ...
    if (map_fd < 0) {
        fprintf(stderr, "Error: state_map not found\n");
        cleanup(0);
        return 1;
    }
